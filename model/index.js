...
const getQuestions = (id) => {
  //TODO Need to return questions that do not have answers, AND need to handle only returning answers that have not been reported
    ...
    return new Promise((resolve, reject) => {
      pool.query(questionQuery, (err, data) => {
        if (err) {
          return reject(err)
        } else {
          resolve(data)
  ...
  const getAnswers = (data) => {
    ...
    return new Promise((resolve, reject) => {
      pool.query(answerQuery, (err, data) => {
        if (err) {
          return reject(err);
        }
        resolve(data);
  ...
  const postQuestion = (question) => {
    ...
    return new Promise((resolve, reject) => {
      pool.query(`INSERT INTO questions (${questionFields}) VALUES (${question.product_id}, "${question.body}", UNIX_TIMESTAMP(), "${question.name}", "${question.email}", 0, 0);`, (err, data) => {
        if (err) {
            return reject(err);
        } else {
  ...
  const postAnswer = (data) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      pool.query(answerQueryString, (err, answerData) => {
        if (err) {
          return reject(err);
        }
        ...
        let promises = data.photos.map((photo) => {
          return new Promise((resolve, reject) => {
            ...
            pool.query(photoQueryString, (err, photoData) => {
              if(err) {
                return reject(err);
              }
              resolve(photoData);
